...
for (const width of [390, 768, 1440]) {
  test(`public catalog works at ${width}px`, async ({ page }) => {
    ...
    await expect(page.locator('.cover-card')).toHaveCount(5);
...
for (const game of ['arena', 'pong']) {
  test(`${game} practice loads, plays, pauses, and restarts`, async ({ page }) => {
    ...
    await page
      .getByRole('link', { name: /Try .* practice/ })
      .first()
      .click();
...
test('touch Arena supports two sticks without a foreign release cancelling movement', async ({
  browser,
}) => {
  ...
  await page
    .locator('.arena-touch__zone--move')
  ...
  await page
    .locator('.arena-touch__zone--turn')
    .dispatchEvent('pointerdown', {
      pointerId: 2,
      pointerType: 'touch',
      clientX: 700,
  ...
  await page
    .locator('.arena-touch__zone--move')
    .dispatchEvent('pointerup', { pointerId: 2, pointerType: 'touch' });
