...
test('Arena practice settings open with P, apply, persist, and remain inside fullscreen', async ({
  page,
}) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () => JSON.parse(localStorage.getItem('littlegames.arena.settings')).look.fieldOfView,
      ),
    )
    .toBe(1.1);
  ...
  await expect(panel).toBeVisible();
  expect(await panel.evaluate((element) => document.fullscreenElement?.contains(element))).toBe(
    true,
  );
...
test('Mobile Arena practice offers joystick and touchpad, keeps settings scrollable and saved', async ({
  browser,
}) => {
...
test('Touchpad stops when the finger stops; joystick keeps turning; settings release held input', async ({
  page,
}) => {
  await page.goto('/');
  const result = await page.evaluate(async () => {
    ...
    container.style.cssText = 'position:fixed;width:800px;height:500px;top:0;left:0';
    ...
    const input = createArenaInput(
      canvas,
      container,
      { onLockChange() {}, onLockRefused() {}, onOpenSettings() {} },
      {
        ...DEFAULT_ARENA_SETTINGS,
        touch: { ...DEFAULT_ARENA_SETTINGS.touch, lookMode: 'touchpad' },
      },
    );
    input.start();
    ...
    send('pointermove', 600);
    const afterSwipe = input.forward();
    input.advance(0.5);
    const afterHold = input.forward();
    send('pointerup', 600);
    input.setSettings(DEFAULT_ARENA_SETTINGS);
    send('pointerdown', 500);
    send('pointermove', 600);
    input.advance(0.2);
    const joystickA = input.forward();
    input.advance(0.2);
    const joystickB = input.forward();
    input.setEnabled(false);
    input.advance(0.5);
    const disabled = input.forward();
    input.stop();
    container.remove();
    window.matchMedia = original;
    return { afterSwipe, afterHold, joystickA, joystickB, disabled };
  });
  expect(result.afterSwipe).toEqual(result.afterHold);
  expect(result.joystickA).not.toEqual(result.joystickB);
  expect(result.disabled).toEqual(result.joystickB);
});
